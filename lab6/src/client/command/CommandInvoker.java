    ...
    public boolean execute(String str, Pack pack) {
        ...
        if (commandWithArgument.containsKey(word[0].toLowerCase(Locale.ROOT))) {
            if (arg.length == 1) {
                ...
                IOUtils.printErr("У команды " + word[0] + " должен быть один аргумент!");
                return false;
            }
            ...
            if (arg.length <= 0) {
                ...
                IOUtils.printErr("У команды " + word[0] + " не должно быть аргументов!");
                return false;
            }
        } else {
            IOUtils.printErr("Команда " + word[0] + " не распознана, введите корректную команду!");
            return false;
        }
    }
}
